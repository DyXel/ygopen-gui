...
inline bool GetDisplayIndexFromWindow(SDL_Window* window, int& displayIndex)
{
...
inline SDL_Window* ConstructWindow()
{
...
#endif // USE_GL_CORE
	// Create window.
	auto sdlWindow = SDL_CreateWindow("YGOpen",
...
inline SDL_GLContext ConstructGLContext(SDL_Window* sdlWindow)
{
	// Create GL context.
	auto sdlGLCtx = SDL_GL_CreateContext(sdlWindow);
	if(sdlGLCtx == nullptr)
	{
...
GameInstance::GameInstance() :
	cfg(),
	sdlWindow(ConstructWindow()),
	sdlGLCtx(ConstructGLContext(sdlWindow)),
	imm(),
	svc({cfg, imm}),
	now(0U),
	then(0U),
...
{
	if(e.type == SDL_QUIT) // TODO: each state should be responsible of exiting
